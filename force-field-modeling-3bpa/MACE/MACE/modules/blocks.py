...
from .efficient_multi_tensor_product import EfficientMultiTensorProduct
...
class EquivariantProductBasisBlock(torch.nn.Module):
    ...
    def forward(
        ...
        node_feats = self.efficient_multi_tensorproduct(node_feats, node_attrs)
        if self.use_complex:
            if self.use_sc:
                return self.linear(node_feats.real) + sc
            else:
                return self.linear(node_feats.real)
        else:
            if self.use_sc:
...
class RealAgnosticInteractionBlock(InteractionBlock):
    def _setup(self,) -> None:
        ...
        # Linear
        irreps_mid = irreps_mid.simplify()
        self.irreps_out = (self.target_irreps * self.num_radial_coupling).simplify()
        self.linear = o3.Linear(
            irreps_mid, self.irreps_out, internal_weights=True, shared_weights=True
        )
        ...
        # Selector TensorProduct
        self.skip_tp = o3.FullyConnectedTensorProduct(
            self.irreps_out, self.node_attrs_irreps, self.hidden_irreps
        )
        self.reshape = reshape_irreps(self.irreps_out)
...
class RealAgnosticResidualInteractionBlock(InteractionBlock):
    def _setup(self,) -> None:
        ...
        # Linear
        irreps_mid = irreps_mid.simplify()
        self.irreps_out = (self.target_irreps * self.num_radial_coupling).sort()[0]
        self.linear = o3.Linear(
            irreps_mid, self.irreps_out, internal_weights=True, shared_weights=True
        )
...
class RealAgnosticNonLinearResidualInteractionBlock(InteractionBlock):
    def _setup(self,) -> None:

        self.embedding_mlp = nn.FullyConnectedNet(
            [self.node_feats_irreps.dim]
            + 3 * [1024]
            + [(self.node_feats_irreps * self.num_radial_coupling).dim],
            torch.nn.functional.silu,
        )
        # First linear
        self.linear_up = o3.Linear(
        ...
        # Selector TensorProduct
        self.skip_tp = o3.FullyConnectedTensorProduct(
            self.node_feats_irreps, self.node_attrs_irreps, self.target_irreps
        )
        self.reshape = reshape_irreps(self.irreps_out)
    ...
    def forward(
        ...
        sender, receiver = edge_index
        num_nodes = node_feats.shape[0]
        node_feats = self.embedding_mlp(node_feats)
        sc = self.skip_tp(node_feats, node_attrs)
        node_feats = self.linear_up(node_feats)
        tp_weights = self.conv_tp_weights(edge_feats)
        node_feats = node_feats.repeat(1, self.num_radial_coupling)
...
class RealElementDependentResidualInteractionBlock(InteractionBlock):
    def _setup(self,) -> None:
