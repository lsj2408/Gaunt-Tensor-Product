...
from MACE.tools.degree import NaiveMaxDeg
...
from MACE.modules.utils import compute_forces, get_edge_vectors_and_lengths
from LieCG.CG_coefficients.CG_rot_real import U_matrix_real


class InvariantMACE(torch.nn.Module):
    def __init__(
        self,
        r_max: float,
        num_bessel: int,
...
class RealNonLinearBodyOrderedModel(torch.nn.Module):
    def __init__(
        self,
        r_max: float,
        num_bessel: int,
        ...
        num_features = hidden_irreps.count(o3.Irrep(0, 1))
        interaction_irreps = (sh_irreps * num_features).sort()[0].simplify()
        coupling_irreps = (sh_irreps * num_radial_coupling).sort()[0].simplify()
...
class RealScaleShiftNonLinearBodyOrderedModel(RealNonLinearBodyOrderedModel):
    def __init__(
        self, atomic_inter_scale: float, atomic_inter_shift: float, **kwargs,
    ):
        super().__init__(**kwargs)
...
class EquivariantNonLinearBodyOrderedModel(torch.nn.Module):
    def __init__(
        self,
        r_max: float,
        num_bessel: int,
...
class EquivariantRealScaleShiftNonLinearBodyOrderedModel(
