...
def test_multiace_model():
    ...
    model = modules.InvariantMACE(**model_config)
...
def test_model_real():
    ...
    model_config = dict(
        ...
        num_radial_coupling=2,
        atomic_energies=atomic_energies,
        avg_num_neighbors=8,
        correlation=3,
    )
    model = modules.RealNonLinearBodyOrderedModel(**model_config)
...
def test_model_equivariant_real():
    atomic_energies = np.array([1.0, 3.0], dtype=float)
    model_config = dict(
        r_max=5,
        num_bessel=8,
    ...
    model = modules.EquivariantNonLinearBodyOrderedModel(**model_config)
